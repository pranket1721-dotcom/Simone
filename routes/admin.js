...
var conn=mysql.createConnection({
    host:'bcqygvtqwzyswzekpoxm-mysql.services.clever-cloud.com',
    user:'uixfddxinj9x9sx9',
    password:'zoliueybiWN6Mo930aLD',
    database:'bcqygvtqwzyswzekpoxm'
})
...
router.get('/work_delete/:id', async (req, res) => {
    var id = req.params.id;

    var sql = "DELETE FROM work WHERE wid=?";
...
router.get('/experience_list',async(req,res)=>{
    ...
    res.render('admin/experience_list.ejs',{experience:data1});
})
...
router.get('/skills_list',async(req,res)=>{
    ...
    res.render('admin/skills_list.ejs',{skill:skill});
})
router.get('/skills_delete/:id', async (req, res) => {
    var id = req.params.id;

    var sql = "DELETE FROM skill WHERE eid=?";
...
router.post('/client_save',async(req,res)=>{
    ...
    var imgname=req.files.c_img.name;
    var newname=Date.now()+imgname;
    var imgpath=path.join(__dirname,'../','public/img',newname);
    img.mv(imgpath,(err)=>{})
    // res.send(newname);
    var sql='insert into client(c_msg,c_img,c_name,c_position)values(?,?,?,?)';
...
router.post('/home_update_save/:id/:img',async(req,res)=>{
    ...
    var imgpath=path.join(__dirname,'../','public/img',myphoto);
    img.mv(imgpath,(err)=>{})
...
router.post('/about_update_save/:id',async(req,res)=>{
     var id=req.params.id;
     var {a_title,a_desc,a_name,a_email,a_age,a_address,a_resume,a_pdf,a_count_experience,a_count_project,a_count_clients,a_count_awards}=req.body;
    //  res.send(req.body);
    var sql='update about set a_title=?,a_desc=?,a_name=?,a_email=?,a_age=?,a_address=?,a_resume=?,a_pdf=?,a_count_experience=?,a_count_project=?,a_count_clients=?,a_count_awards=? where aid=?';
    var data=await exe(sql,[a_title,a_desc,a_name,a_email,a_age,a_address,a_resume,a_pdf,a_count_experience,a_count_project,a_count_clients,a_count_awards,id]);
...
router.get('/contact_update',async(req,res)=>{
    var sql='select * from contact where id=1';
    data=await exe(sql);
    // res.send(data[0]);

    res.render('admin/contact_update.ejs',{data:data[0]});
})

router.post('/contact_update_save/:id',async(req,res)=>{
     var id=req.params.id;
     var {address,mobile,email,symbol}=req.body
    var sql='update contact set address=?,mobile=?,email=?,symbol=? where id=?';
    var data=await exe(sql,[address,mobile,email,symbol,id]);
        res.redirect('/admin/contact_update');
